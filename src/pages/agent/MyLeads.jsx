...
export default function MyLeads() {
  ...
  const baseLeads = useMemo(() => {
    ...
    // Agent scoping — only own assigned leads
    leads = leads.filter((l) => l.assignedAgent === user.name);
    if (statusFilter !== 'All') {
      leads = leads.filter((l) => l.status === statusFilter);
    }
    return leads;
  }, [activeWebsiteId, user.name, statusFilter]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-xl font-semibold text-brand-ink">My Leads</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 xl:grid-cols-4">
        <div className="card flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-50 text-brand-magenta">
            <Users size={18} />
          </span>
          <div>
        ...
        <div className="card flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-brand-purple">
        ...
        <div className="card flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-display text-lg font-semibold text-brand-ink">NA</p>
          </div>
        </div>
      </div>
      ...
      <div className="h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-panel">
        ...
        <div className="mt-8">
          ...
          <p className="mt-3 rounded-xl bg-brand-mist p-4 text-sm text-brand-ink/50">
            No calls logged yet. Once connected to your telephony provider, call
            recordings and notes will appear here.
          </p>
        </div>
      </div>
    </div>
