...
import {
  ...
  Play, PhoneIncoming, PhoneOutgoing, Mic, Target, TrendingUp,
} from 'lucide-react';
...
export default function MyLeads() {
  ...
  const baseLeads = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      leads = leads.filter((l) => {
        const hay = `${l.name} ${l.mobile} ${l.account} ${l.category}`.toLowerCase();
        return hay.includes(q);
      });
    }
    return leads;
  }, [activeWebsiteId, user.name, statusFilter, searchQuery]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      won,
      conversion: all.length > 0 ? Math.round((won / all.length) * 100) : 0,
    };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-brand-ink/50">
            Customer details assigned to you — call, message, and log outcomes.
          </p>
        </div>
      ...
      <div className="card !p-0">
        ...
        <div className="border-b border-brand-lilac/60 p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="ml-auto flex items-center gap-2 rounded-lg border border-brand-lilac bg-white px-3 py-1.5 text-xs font-semibold text-brand-ink">
              Total:
...
function LeadDrawer({ lead, websiteName, onClose }) {
  const recordings = [
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="rounded-xl border border-brand-lilac bg-brand-mist/40 p-4">
            ...
            <p className="text-sm italic leading-relaxed text-brand-ink/70">
              "Agent: Good afternoon, this is {lead.assignedAgent || 'your agent'} from {websiteName}. Am I speaking with {lead.name.split(' ')[0]}?"
              <br />
