// src/pages/agent/LiveCalls.jsx
import { useMemo, useState } from 'react';
import {
  PhoneIncoming, PhoneOutgoing, PhoneMissed, Users, PhoneCall,
  Clock, User, Mic, Play, X, Headphones,
} from 'lucide-react';
import StatCard from '../../components/common/StatCard';
import { useAuth } from '../../context/AuthContext';

/* Demo live calls — scoped to this agent */
const SAMPLE_LIVE_CALLS = [
  { id: 'L-A01', agentName: 'Agent1', customer: 'Ramesh Kannan', mobile: '9876543210', type: 'incoming', status: 'ringing', duration: '0:12' },
  { id: 'L-A02', agentName: 'Agent1', customer: 'Venu Gopal', mobile: '9876543213', type: 'outgoing', status: 'connected', duration: '4:32' },
];
...
export default function LiveCalls() {
  ...
  /* Only show calls for this agent */
  const liveCalls = useMemo(
    () =>
      SAMPLE_LIVE_CALLS.filter(
        (c) => c.agentName === user.name && c.type === tab
      ),
    [user.name, tab]
  );

  const stats = useMemo(() => {
    const calls = SAMPLE_LIVE_CALLS.filter((c) => c.agentName === user.name);
    return {
      total: calls.length,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-brand-ink/50">
            Your active calls in real time.
          </p>
        </div>
      ...
      {liveCalls.length === 0 ? (
        <div className="card flex min-h-[220px] flex-col items-center justify-center gap-3 text-center">
          ...
          <p className="max-w-sm text-sm text-brand-ink/50">
            When a call connects on your IVR line, it will appear here in real
            time with caller ID, duration, and status.
          </p>
        </div>
      ) : (
      ...
      <div className="h-full w-full max-w-md overflow-y-auto bg-white shadow-panel">
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <InfoBox label="Agent" value={call.agentName || 'You'} />
          </div>
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-brand-ink/40">
                <Headphones size={12} /> Past Recordings
              </p>
