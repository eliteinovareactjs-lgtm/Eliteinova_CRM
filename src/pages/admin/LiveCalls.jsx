    ...
    <div className="space-y-6">
      ...
      <div className="flex w-fit gap-6 border-b border-brand-lilac text-sm font-semibold text-brand-ink/40">
        <button
          ...
          className={`pb-3 ${tab === 'incoming' ? 'border-b-2 border-brand-magenta text-brand-magenta' : ''}`}
        >
          Incoming
        </button>
        <button
          onClick={() => setTab('outgoing')}
          className={`pb-3 ${tab === 'outgoing' ? 'border-b-2 border-brand-magenta text-brand-magenta' : ''}`}
        >
          Outgoing
        </button>
      </div>
      ...
      <div className="card flex min-h-[220px] flex-col items-center justify-center gap-3 text-center">
        ...
        <p className="max-w-sm text-sm text-brand-ink/50">
          When a call connects on the active IVR number, it will appear here in real time
          with caller ID, agent, and duration.
        </p>
      </div>
    </div>
  );
